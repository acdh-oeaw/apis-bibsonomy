...
class ReferenceNewForm(ModelForm):
    bibs_url = ModelChoiceField(
            queryset=ZoteroEntry.objects.exclude(data__data__deleted__isnull=False),
            widget=Selectize(search_lookup="data__data__title__icontains")
    )
    class Media:
